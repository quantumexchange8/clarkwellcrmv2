...
@section('contents')
    <h1 class="font-semibold text-2xl text-gray-500">News</h1>
                ...
                <div class="max-[755px]:flex max-[755px]:flex-col gap-2">
                    ...
                    <a href="{{ route('create_news') }}" class="inline-flex text-white py-1.5 px-8 rounded bg-[#2AC769] hover:bg-success-500 text-lg font-bold float-right items-center justify-center">
                        <div class="flex items-center">
                            ...
                            <span class="ml-4 text-md font-bold">Create News</span>
                        </div>
                    </a>
                </div>
            </div>
                ...
                <tbody>
                    @foreach($news_all as $news)
                        <tr class="border-b odd:bg-[#FDFCF3] even:bg-[#F6F6F6]">
                            ...
                            <td class="px-6 py-4">
                                @if($news->visibility)
                                    <span class="font-semibold text-success">Visible</span>
                                @else
                                    <span class="font-semibold text-danger">Not Visible</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    <a href="#viewModal-{{ $news->id }}"
                                       data-modal-target="viewModal-{{ $news->id }}" data-modal-toggle="viewModal-{{ $news->id }}"
                                       data-te-ripple-init
    ...
    @else
        <div class="flex p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
            ...
            <div>
                <span class="font-medium">No Record Found!</span> Create News to have more records.
            </div>
        </div>
    @endif
