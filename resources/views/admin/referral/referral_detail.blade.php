...
@section('contents')
    ...
    <div class="mt-8 rounded-lg dark:border-gray-700">
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div class="flex flex-col p-4 text-center justify-between items-center bg-[#FDFCF3] border border-orange-300 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-orange-100 hover:shadow-lg dark:border-orange-700 dark:bg-orange-800 dark:hover:bg-orange-700">
                <div class="inline-flex items-center my-4 justify-center w-20 h-20 overflow-hidden bg-orange-400 rounded-full shrink-0 grow-0 dark:bg-orange-600">
                ...
                <div class="flex flex-col mx-4">
                    <h2 class="font-semibold text-lg text-[#FFA168] mb-2">{{ $user->name }}</h2>
                    <span class="text-sm text-gray-500">
                            {{ $user->email }}
                    </span>
                </div>
                <div>
                    <h2 class="font-semibold text-lg text-[#FFA168] mb-2">{{ $user->rank->name }}</h2>
                    <span class="text-sm text-gray-500">
                        @lang('public.rank')
                    </span>
                </div>
            </div>
            <div class="flex flex-col p-10 items-center justify-between text-center bg-[#FDFCF3] border border-orange-300 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-orange-100 hover:shadow-lg dark:border-orange-700 dark:bg-orange-800 dark:hover:bg-orange-700">
                <div class="flex flex-col mx-4">
        ...
        <table class="w-full text-md text-left text-gray-700 dark:text-gray-400">
            ...
            @foreach($brokers as $broker)
                <tr class="border-b border-gray-200 dark:border-gray-700">
                    <th scope="row" class="px-6 py-4 font-medium text-[#FFA168] whitespace-nowrap bg-[#FDFCF3] dark:text-white dark:bg-gray-800">
                        {{$broker->name}}
                    </th>
                    <td class="px-6 py-4">
            ...
            <tr class="border-t-4 border-gray-200 dark:border-gray-700 font-bold text-lg">
                <th scope="row" class="px-6 py-4 font-bold text-[#FFA168] whitespace-nowrap bg-[#FDFCF3] dark:text-white dark:bg-gray-800">
                    @lang('public.total'):
                </th>
                <td class="px-6 py-4">
