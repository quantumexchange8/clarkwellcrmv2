...
@section('contents')
    ...
    <div class="flex flex-row gap-4 max-[1150px]:block">
        <h1 class="flex-1 font-semibold text-2xl text-gray-600">Members / {{ $user->name }}</h1>
        <a href="{{ route('member_deposit', $user->id) }}" class="mt-2 bg-[#1AB759] hover:bg-green-400 border border-green-200 focus:ring-4 focus:outline-none focus:ring-green-600 text-card-14 rounded-lg px-5 py-2.5 text-center inline-flex items-center ">
            ...
            <span class="ml-4 text-white">View Deposit</span>
        </a>
        <a href="{{ route('member_edit', $user->id) }}" class="mt-2 bg-[#4DA5FF]  hover:bg-blue-600 border border-blue-200 focus:ring-4 focus:outline-none focus:ring-blue-600 font-semibold text-md text-gray-500 rounded-lg px-5 py-2.5 text-center inline-flex items-center ">
            ...
            <span class="ml-4 text-white">Edit</span>
        </a>
        ...
        <div class="grid grid-cols-4 items-start rounded-xl bg-[#FDFCF3] p-4 shadow-lg w-full max-[1150px]:grid-cols-3">
            ...
            <div class="col-span-1 flex-col items-center py-8 border-r-4 border-orange-500 h-full px-12 text-center max-[1150px]:col-span-3 max-[1150px]:border-none">
                ...
                <div class="items-center mt-4 md:mt-6">
                    <span class="font-semibold text-lg text-orange-400 mt-4">Last Rank Up:</span> <br>
                    <span class="font-semibold text-lg text-gray-600">{{\Carbon\Carbon::parse($user->rank_update_at)->format('Y-m-d')}} {{$user->auto_rank_up ? '(Automatic)' : '(Manual)'}}</span>
                </div>
            ...
            <div class="col-span-3 ml-4 p-4 md:mt-6">
                <div class="grid grid-cols-2 gap-8 break-words max-[800px]:grid-cols-1">
                    <div class="px-4">
                        <h2 class="font-semibold text-lg text-orange-400 mb-4">Email</h2>
                        <span class="font-semibold text-lg text-gray-500">
                            {{ $user->email }}
                        </span>
                    </div>
                    <div class="px-4">
                        <h2 class="font-semibold text-lg text-orange-400 mb-4">Contact</h2>
                        <span class="font-semibold text-lg text-gray-500">
                            {{ $user->contact_number }}
                        </span>
                    </div>
                    <div class="mt-10 px-4">
                        <h2 class="font-semibold text-lg text-orange-400 mb-4">Address</h2>
                        <span class="font-semibold text-lg text-gray-500">
                            {{ $user->address }}
                        </span>
                    </div>
                    <div class="mt-10 px-4">
                        <h2 class="font-semibold text-lg text-orange-400 mb-4">Country</h2>
                        <span class="font-semibold text-lg text-gray-500">
                            {{ $user->country }}
                        </span>
                    </div>
                    <div class="mt-10 px-4">
                        <h2 class="font-semibold text-lg text-orange-400 mb-4">Date Joined</h2>
                        <span class="font-semibold text-lg text-gray-500">
                            {{ $user->created_at }}
                        </span>
                    </div>
                    <div class="mt-10 px-4">
                        <h2 class="font-semibold text-lg text-orange-400 mb-4">Status</h2>
                        @if($user->status == 1)
                            <span class="text-success font-semibold text-lg text-gray-500">Active</span>
                        @elseif($user->status == 2)
                            <span class="text-warning font-semibold text-lg text-gray-500">Inactive</span>
                        @elseif($user->status == 3)
                            <span class="text-danger font-semibold text-lg text-gray-500">Suspended</span>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="p-6 space-y-6">
                    <form method="post" action="{{ route('member_details', $user->id) }}">
                        ...
                        <div class="mb-6">
                            <label for="rank" class="block mb-2 text-lg text-[#FFA168]">Change Ranking</label>
                        ...
                        <div class="mb-6">
                            <label class="relative inline-flex items-center cursor-pointer">
                                ...
                                <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">Auto Rank Up</span>
                            </label>
                        </div>
                        <div class="text-center">
                            <button type="submit" class="text-white bg-[#FFA168] hover:bg-orange-400 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm w-full sm:w-auto px-10 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center justify-center">Confirm</button>
                        </div>
                    </form>
                </div>
            </div>
