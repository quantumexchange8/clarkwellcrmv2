...
@section('contents')
        <nav class="flex mb-4" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3 text-lg font-semibold">
                ...
                <div class="flex items-center">
                    ...
                    <p href="#" class="ml-1  text-gray-700 hover:text-orange-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">@lang('public.fund')</p>
                </div>
                </li>
            </ol>
        </nav>
        <div class="grid grid-cols-6 w-auto max-[1250px]:grid-cols-4">
            <div class="col-span-4 px-4">
                <form action="{{ route('funds_listing') }}" method="post" class="flex p-4 max-[900px]:flex-col gap-3">
                    @csrf
                ...
                <!-- Help text -->
                    <span class="text-sm text-gray-700 dark:text-gray-400">
                        @if(count($deposits) > 0)
                            @lang('public.showing') <span class="font-semibold text-gray-900 dark:text-white">{{$deposits->count()}}</span> @lang('public.to') <span class="font-semibold text-gray-900 dark:text-white">{{ $deposits->count() }}</span> @lang('public.of') <span class="font-semibold text-gray-900 dark:text-white">{{ $deposits->total() }}</span> @lang('public.entries')
                        @else
        ...
        <div class="col-span-2 px-4 max-[1250px]:col-span-4 max-[1250px]:pt-8">
            <div class="max-w-md  bg-[#FDFCF3] border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mx-auto text-center pb-4">
                <div class="py-12 bg-orange-500 w-full rounded-lg">
                    <h5 class="text-3xl font-semibold tracking-tight text-slate-50">@lang('public.current_fund'): </h5>
                    <h5 class=" text-2xl font-semibold tracking-tight text-slate-50">$ {{number_format($personal_total,2)}}</h5>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="rounded-full -mt-10 w-20 h-20 mx-auto text-orange-500 bg-[#FDFCF3] p-2 drop-shadow-2xl mb-2" xmlns="http://www.w3.org/2000/svg">
                    <path d="M10.464 8.746c.227-.18.497-.311.786-.394v2.795a2.252 2.252 0 01-.786-.393c-.394-.313-.546-.681-.546-1.004 0-.323.152-.691.546-1.004zM12.75 15.662v-2.824c.347.085.664.228.921.421.427.32.579.686.579.991 0 .305-.152.671-.579.991a2.534 2.534 0 01-.921.42z" />
                    <path fill-rule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zM12.75 6a.75.75 0 00-1.5 0v.816a3.836 3.836 0 00-1.72.756c-.712.566-1.112 1.35-1.112 2.178 0 .829.4 1.612 1.113 2.178.502.4 1.102.647 1.719.756v2.978a2.536 2.536 0 01-.921-.421l-.879-.66a.75.75 0 00-.9 1.2l.879.66c.533.4 1.169.645 1.821.75V18a.75.75 0 001.5 0v-.81a4.124 4.124 0 001.821-.749c.745-.559 1.179-1.344 1.179-2.191 0-.847-.434-1.632-1.179-2.191a4.122 4.122 0 00-1.821-.75V8.354c.29.082.559.213.786.393l.415.33a.75.75 0 00.933-1.175l-.415-.33a3.836 3.836 0 00-1.719-.755V6z" clip-rule="evenodd" />
                </svg>
